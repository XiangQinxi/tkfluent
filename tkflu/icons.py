"""窗口 / 任务栏图标。

图标以 base64 PNG 的形式内嵌在本模块里，因此不依赖任何资源文件。

旧实现的问题
------------
``icon()`` / ``light()`` / ``dark()`` 每次调用都会 ``mkstemp()`` 一个**新的**临时
文件，而且 **从不关闭 mkstemp 返回的文件描述符**：

* 每创建一个窗口就泄漏一个句柄（Windows 上有句柄上限）；
* 每创建一个窗口就在系统临时目录里留下一个残留文件；
* 而这些图标数据本来就是固定的，完全没必要每次重新解码、重新落盘。

现在的做法
----------
* :func:`icon_photoimage` —— **推荐**。直接用 base64 建 ``PhotoImage``，
  完全不碰磁盘（Tk 8.6 原生支持 PNG）。
* :func:`icon` / :func:`light` / :func:`dark` —— 保持"返回文件路径"的历史行为，
  但按图标名缓存路径：进程内每个图标只落盘一次，并且关闭 fd。
"""

from __future__ import annotations

import os
from base64 import b64decode
from tempfile import mkstemp

__all__ = [
    "icon",
    "light",
    "dark",
    "icon_photoimage",
    "icon_data",
]

_ICON_B64 = (
    b""
)

_LIGHT_B64 = (
    b""
)

_DARK_B64 = (
    b""
)

#: 图标名 -> base64 数据
_DATA = {
    "icon": _ICON_B64,
    "light": _LIGHT_B64,
    "dark": _DARK_B64,
}

#: 图标名 -> 已落盘的临时文件路径（进程内复用）
_FILES: "dict[str, str]" = {}
#: (Tk 解释器, 图标名) -> PhotoImage
_PHOTOS: "dict[tuple, object]" = {}


def _icon_bytes(name: str) -> bytes:
    try:
        return _DATA[name]
    except KeyError:
        raise ValueError(
            f"未知的图标 {name!r}；可用：{sorted(_DATA)}"
        ) from None


def icon_data(name: str = "light") -> bytes:
    """返回图标的 base64 数据（不落盘）。"""
    return _icon_bytes(name)


def _materialize(name: str) -> str:
    """把图标解码写入一个进程内复用的临时文件，返回路径。

    同一个图标无论被请求多少次，都只会创建一次文件、且 fd 一定被关闭。
    """
    path = _FILES.get(name)
    if path is None:
        fd, path = mkstemp(suffix=".png", prefix="tkfluent.icon.")
        os.close(fd)  # 必须关闭，否则每建一个窗口泄漏一个句柄
        with open(path, "wb") as handle:
            handle.write(b64decode(_icon_bytes(name)))
        _FILES[name] = path
    return path.replace("\\", "//")


def icon() -> str:
    """默认应用图标的文件路径（历史 API，见模块文档）。"""
    return _materialize("icon")


def light() -> str:
    """浅色主题图标的文件路径（历史 API，见模块文档）。"""
    return _materialize("light")


def dark() -> str:
    """深色主题图标的文件路径（历史 API，见模块文档）。"""
    return _materialize("dark")


def icon_photoimage(name: str = "light", master=None):
    """直接用内嵌 base64 建 ``PhotoImage``，**完全不落盘**。

    ``master`` 应当传入目标控件：不传时 ``tkinter`` 会挂到默认根窗口上，
    多 Tk 解释器场景下会出现"这个图片不属于该解释器"的错误。
    """
    from tkinter import PhotoImage

    tk = getattr(master, "tk", master)
    key = (id(tk) if tk is not None else 0, name)
    photo = _PHOTOS.get(key)
    if photo is None:
        photo = PhotoImage(data=_icon_bytes(name), master=master)
        _PHOTOS[key] = photo
    return photo
