...
dwg.add(
    dwg.rect((x1, (y2 - y1) / 2 - 3), (x2 - x1, (y2 - y1) / 2 + 3), rx=3, fill=fill)
)
...
dwg.add(dwg.circle((x, y), r1, fill=stroke, fill_opacity=1, fill_rule="evenodd"))
dwg.add(
    dwg.circle(
        (x, y), r1 - 1, fill=fill, fill_opacity=fill_opacity, fill_rule="nonzero"
    )
)
dwg.add(
    dwg.circle(
